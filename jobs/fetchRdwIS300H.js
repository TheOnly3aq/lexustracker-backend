...
const ensureMongoConnection = async () => {
  ...
  try {
    ...
    console.log("MongoDB connected for IS300H job");
  } catch (err) {
    console.error("Failed to connect to MongoDB for IS300H job:", err);
    throw err;
  }
};
  ...
  try {
    ...
    for (const entry of entries) {
      try {
        ...
        console.warn(
          `(IS300H) Skipped entry ${entry.kenteken}:`,
          entryError.message
        );
      }
    }
    ...
    console.log(
      `(IS300H) Saved RDW count for ${dateString}: ${entries.length}`
    );
...
// Export the function and cron schedule setup
const scheduleIS300HJob = () => {
  console.log("Scheduling IS300H cron job...");
  cron.schedule("0 0 * * *", fetchRdwDataIS300H);
};

module.exports = { fetchRdwDataIS300H, scheduleIS300HJob };