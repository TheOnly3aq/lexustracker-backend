...
const ensureMongoConnection = async () => {
  ...
  console.log("MongoDB connected for fetchRdw job");
};

const fetchRdwData = async () => {
  try {
    await ensureMongoConnection();
    console.log("(IS250C) Starting RDW data fetch...");
    ...
    console.log(`(IS250C) Fetched ${entries.length} entries from RDW API`);
    ...
    for (const entry of entries) {
      try {
        ...
        console.warn(
          `(IS250C) Skipped entry ${entry.kenteken}:`,
          entryError.message
        );
      }
    }
    ...
    console.log(
      `(IS250C) Saved RDW count for ${dateString}: ${entries.length}`
    );
    console.log(`(IS250C) Saved RDW count for ${yearMonth}: ${entries.length}`);
    console.log(
      `(IS250C) Cached data: ${newCount} new entries, ${updatedCount} updated entries`
    );
    ...
    if (deleteResult.deletedCount > 0) {
      console.log(
        `(IS250C) Removed ${deleteResult.deletedCount} entries that are no longer in the API`
      );
    }
    ...
    if (totalChanges > 0) {
      ...
      console.log(
        `(IS250C) Daily differences for ${dateString}: ${addedKentekens.length} added, ${removedKentekens.length} removed`
      );
    } else {
      await DailyDifference.findOneAndUpdate(
        { date: dateString },
      ...
      console.log(`(IS250C) No changes detected for ${dateString}`);
    }
  } catch (err) {
    console.error("(IS250C) Error fetching RDW data:", err.message);
  }
};
